...
function updateExerciseFilterOptions() {
    ...
    if (seen.has(currentValue)) select.value = currentValue;
}
...
function updateStats() {
    ...
    Object.values(allWorkouts).forEach(sets => {
        sets.forEach(s => {
            totalVolume += s.weight * s.reps * s.sets;
            totalSets += s.sets;
            exercises.add(s.exercise);
        });
    });

    document.getElementById("statTotalVolume").textContent = totalVolume;
    document.getElementById("statTotalSets").textContent = totalSets;
    document.getElementById("statExercises").textContent = exercises.size;
}
...
function buildVolumeData() {
    ...
    Object.entries(allWorkouts).forEach(([date, sets]) => {
        let vol = 0;
        sets.forEach(s => vol += s.weight * s.reps * s.sets);
        map[date] = vol;
    });
...
function buildDailyBestWeightData() {
    ...
    Object.entries(allWorkouts).forEach(([date, sets]) => {
        let best = 0;
        sets.forEach(s => { if (s.weight > best) best = s.weight; });
        map[date] = best;
    });
...
function addSet() {
    ...
    if (prMessage) alert(`🔥 ${prMessage} on ${exercise}!`);
}
